...
<html>
 ...
 <body>
  ...
  <div class="container">
  	<h3>步骤修改</h3>
  	<hr>
	<form id ="N_Form" class="form-horizontal" action="${pageContext.request.contextPath}/etl/procedure/editStepOracle.do" method="post">
		<div class="row">
		...
		<div class="row">
			...
			<div class="control-group span14">
				<label class="control-label"><s>*</s>数据库选择：</label>
				<div class="controls">
					<select data-rules="{required:true}" id="sjlx" name="serverName" style="width:auto;">
					</select>
				</div>
			</div>
		</div>
		<div class="row">
			<div class="control-group span15">
			  	<label class="control-label">预处理脚本：</label>
			  	<div class="controls control-row4">
			    	<textarea name="prepareSql" id="prepareSql" class="input-large" type="text"></textarea>
			  	</div>
			</div>
		</div>
		<div class="row">
			<div class="control-group span15">
			  	<label class="control-label">后处理脚本：</label>
			  	<div class="controls control-row4">
			    	<textarea name="postSql" id="postSql" class="input-large" type="text"></textarea>
			  	</div>
			</div>
		</div>
		<div class="row">
			<div class="control-group span15">
			  	...
			  	<div class="controls control-row4">
			    	<textarea name="columns" id="columns" data-tip='{"text":"逗号分隔"}' class="input-large" type="text"></textarea>
			  	</div>
			</div>
		</div>
		<div class="row">
			<div class="control-group span8">
				<label class="control-label">入库表名：</label>
				<div class="controls">
					<input name="tableName" id="tableName" type="text" class="input-normal control-text">
				</div>
			</div>
			<div class="control-group span8">
				<label class="control-label">字段分隔符：</label>
				<div class="controls">
					<input name="dataSplit" id="dataSplit" type="text" data-tip='{"text":"不填时默认转换分隔符#_#"}' class="input-normal control-text">
				</div>
			</div>
		</div>
		<div class="row">
			<div class="control-group span16">
				<label class="control-label">入库编码：</label>
				<div class="controls">
					<input name="encoding" id="encoding" type="text" class="input-normal control-text">&nbsp;ZHS16GBK/AL32UTF8...
				</div>
			</div>
		</div>
		<div class="row">
	...
	var jldata = [];
  	...
  	function oraclelist(value){
  	 ...
  	 $(document).ready(function(){
  		...
  		$.ajax({
  			type:'post',
  			url:'${pageContext.request.contextPath}/etl/procedure/getOracleLoadStep.do?stepId=${step.stepId}',
  			dataType:'json',
  			success:function(data){
  				oraclelist(data["serverName"]);
  				$("#prepareSql").val(data["prepareSql"]);
  				$("#postSql").val(data["postSql"]);
  				$("#tableName").val(data["tableName"]);
  				$("#columns").val(data["columns"]);
  				$("#dataSplit").val(data["dataSplit"]);
