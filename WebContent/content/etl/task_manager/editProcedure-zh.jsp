...
<html>
 ...
 <body>
  ...
  <div class="container">
  	<h3>步骤修改</h3>
  	<hr>
	<form id ="N_Form" class="form-horizontal" action="${pageContext.request.contextPath}/etl/procedure/editStepzh.do" method="post">
		<div class="row">
		...
		<div class="row">
			...
			<div class="control-group span8">
				<label class="control-label"><s>*</s>父类：</label>
				<div class="controls">
					<input name="superClass" id="superClass" type="text" data-rules="{required:true}" class="input-normal control-text" />
				</div>
			</div>
		</div>
		<div class="row">
			<div class="control-group span16">
				...
				<div class="controls">
					<input name="charset" id="charset" data-tip='{"text":"不填时默认为GBK"}' type="text" class="input-normal control-text" />
				</div>
			</div>
		</div>
		<div class="row">
	...
	var jldata = [];
  	...
  	function contains(arrlist,obj){
  	 ...
  	 $(document).ready(function(){
  		...
  		$.ajax({
  			type:'post',
  			url:'${pageContext.request.contextPath}/etl/procedure/getConvertStep.do?stepId=${step.stepId}',
  			dataType:'json',
  			success:function(data){
  				$("#superClass").val(data["superClass"]);
