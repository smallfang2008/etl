      ...
      <form id="searchForm" class="form-horizontal span28">
        ...
        <div class="row">
          ...
          <div class="control-group span16">
            ...
            <div class="controls" >
              <input type="text" class="control-text" name="nodeCode" id="nodeCode"><span style="color: red;">&nbsp;*集群和节点必填一个，填节点时不取集群的值</span>
            </div>
          </div>
  ...
  <script type="text/javascript">
  	var Grid = BUI.Grid,
  		...
  		columns = [
            ...
            { title: '指定位置', width:100, dataIndex: 'runPosition'},
    ...
    var store = new Store({
            url : '${pageContext.request.contextPath}/etl/task/taskWaitPageQuery.do',
            autoLoad:false,
            pageSize:10
          }),
        grid = new Grid.Grid({
            ...
            // 底部工具栏
            tbar:{
             items:[{text : '<i class="icon-remove"></i>全部删除',btnCls : 'button button-small',
             			handler:function(){
             				BUI.Message.Alert('暂未开发！',function(){},"info");
						}
					},
		            {text : '<i class="icon-remove"></i>删除',btnCls : 'button button-small',handler : deleteTask}]
            },
            // 顶部工具栏
            bbar : {
              //items 也可以在此配置
	    ...
	    function deleteItems(items){
	      ...
	      BUI.each(items,function(item){
	        ids.push(item.runPositionId+"/"+item.stepId+"/"+item.taskSeries+"_"+item.cmdId);
	      });
	      if(ids.length){
	        BUI.Message.Confirm('确认要删除选中任务？',function(){
		          $.ajax({
		            url : '${pageContext.request.contextPath}/etl/task/deleteWaitTask.do',
		            dataType : 'text',
		            method:"post",
		            cache: false,
		            data:{taskPath:ids+""},
