...
<html>
 ...
 <body>
  ...
  <div class="container">
  	<h3>集群编辑</h3>
  	<hr>
    <form id ="SC_Form" class="form-horizontal" action="${pageContext.request.contextPath}/etl/cluser/editServerCluster.do" method="post">
      <input name="serverClusterId" value="${editObj.serverClusterId}" type="hidden" />
      <!--    -->
      <div class="row">
        <div class="control-group span20">
          <label class="control-label" style="width: 130px;"><s>*</s>集群名称：</label>
          <div class="controls">
            <input name="serverClusterName" type="text" value="${editObj.serverClusterName }" class="control-text" data-rules="{required:true}" style="width: 200px;" onblur="checkExistClusterName(this);">
          </div>
      ...
      <div class="row">
        <div class="control-group span20">
          <label class="control-label" style="width: 130px;"><s>*</s>根目录：</label>
          <div class="controls">
            <input name="rootPath" type="text" value="${editObj.rootPath }" class="control-text" data-rules="{required:true}" style="width: 200px;">
          </div>
        </div>
      </div>
      <div class="row">
        <div class="control-group span20">
          <label class="control-label" style="width: 130px;"><s>*</s>ZOOKEEPER集群：</label>
          <div class="controls" id="zookeeperCluserCode">
          	<input type="hidden" id="zookeeperCluserCodeHide" value="${editObj.zookeeperCluster.zookeeperCode }" name="zookeeperCluster.zookeeperCode" data-rules="{required:true}">
          </div>
        </div>
      </div>
  ...
  <script type="text/javascript">	
  	...
  	var bfh = new BUI.Form.HForm({
        ...
        callback : function(data){
        	if(data==1){
        		BUI.Message.Alert("集群编辑成功!",function(){
					location.href="${pageContext.request.contextPath}/content/etl/server_farm/cluserList.jsp";
				},"success");
        		//history.back();
        	}else{
        		var tip = data;
            	if ("101" == data || data == "106") {
            		tip = "非法的操作！";
            	} else if ("103" == data) {
            		tip = "跟目录为空";
            	} else if ("104" == data) {
        		...
        		BUI.Message.Alert("集群编辑失败!<br>失败信息："+tip,function(){},"error");
        	}
  	...
  	function checkExistClusterName(obj) {
		var objV = $(obj).val();
		//alert("${editObj.serverClusterName }");
		if ("" == objV || "${editObj.serverClusterName }" == objV) {
			$("#clusterNameTip").text("");
			return;
		}
		$.ajax({
